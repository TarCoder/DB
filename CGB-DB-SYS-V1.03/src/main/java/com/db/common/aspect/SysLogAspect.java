...
public class SysLogAspect {
	...
	private void saveLog(ProceedingJoinPoint jp,long time) throws NoSuchMethodException, SecurityException{
		...
		MethodSignature ms=(MethodSignature)jp.getSignature();//方法
		
		Class<?>[] parameterTypes = ms.getParameterTypes();
		Method met = target.getClass().getDeclaredMethod(ms.getName(), parameterTypes);
		RequiredLog annotation = met.getDeclaredAnnotation(RequiredLog.class);
